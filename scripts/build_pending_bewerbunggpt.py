#!/usr/bin/env python3
"""Build scripts/pending_tools.json with BewerbungGPT (German job-application AI)."""
import json
...
TOOLS = [
    {
        'slug': 'bewerbunggpt', 'name': 'BewerbungGPT',
        'vendor': 'CryptoAssets', 'category': 'produktivitaet',
        'tagline': 'Deutscher KI-Bewerbungsassistent — erstellt aus persönlichen Angaben und Stellenanzeigen individuelle Anschreiben (DIN 5008), Lebensläufe und Bewerbungsfotos, speziell für den deutschsprachigen Bewerbungsmarkt.',
        'price': 'Kostenlos (begrenzt) · Einmalpaket 7,99 € · Premium 12,99 €/Mon. oder 79,99 €/Jahr',
        'api': False, 'dsgvo': 'ja', 'origin': 'Deutschland', 'rating': 4.4, 'reviews': 150,
        'pros': [
        ...
        'usecases': [
            'Anschreiben passend zur Stellenanzeige', 'Lebenslauf gestalten und ATS-prüfen',
            'Bewerbungsfoto aus einem Selfie erzeugen', 'Bewerbungen organisieren und nachverfolgen',
        ],
        'launched': '2024-09-01', 'lastUpdated': '2026-08-10',
        'website': 'https://bewerbunggpt.de', 'domain': 'bewerbunggpt.de',
        'stealth': False,
        'cover_cue': 'A hand-drawn document sheet with neat text lines and a small portrait-photo rectangle clipped to its corner, a single magenta accent on the photo frame — an AI-built job application.',
        'features': """- **Anschreiben-Generator**: gleicht Stellenanzeige mit dem Profil ab, formatiert nach **DIN 5008**, Export als PDF/Word (erstes Anschreiben ohne Anmeldung).
- **Lebenslauf-Editor**: 25 Vorlagen in fünf Stilrichtungen, 16 Akzentfarben, Live-Vorschau (Download frei mit Wasserzeichen, Premium ohne).
- **Bewerbungsfotos**: Selfie hochladen, Kleidung/Hintergrund wählen, professionelles Foto erhalten — das **Selfie wird nicht gespeichert**, nur das Ergebnis.
- **Lebenslauf-Check**: neun Kriterien inkl. ATS-Tauglichkeit im Ampelsystem.
- **KI-Assistent** mit Firmen-Web-Recherche (Premium).
- **Bewerbungs-Tracker**: Status, Sendedatum, Gesprächstermine.
- **Eingabe** per Text-Einfügen oder Stellenanzeigen-URL.""",
        'pricing': """- **Kostenlos** · 1 Anschreiben als Gast, 3 weitere nach Anmeldung; 25 CV-Vorlagen (mit Wasserzeichen); 1 Lebenslauf-Check; KI-Assistent mit Monatskontingent.
- **Einmalpaket** · 7,99 € einmalig — 5 Anschreiben mit Feinschliff + Export, 1 voller CV-Check, 1 Download ohne Wasserzeichen, 1 KI-Foto (12 Monate gültig).
- **Premium** · 12,99 €/Mon. oder 79,99 €/Jahr — unbegrenzte Anschreiben, CV-Editor voll, unbegrenzte Checks, KI-Assistent unbegrenzt, 3 KI-Fotos/Mon.; jederzeit kündbar.""",
        'overview': """**BewerbungGPT** ist ein **deutscher KI-Bewerbungsassistent**, der den gesamten Kern einer Bewerbung abdeckt: **Anschreiben, Lebenslauf und Bewerbungsfoto** — speziell zugeschnitten auf den deutschsprachigen Arbeitsmarkt mit seinen eigenen Konventionen. Betrieben wird das Tool von einem Einzelunternehmer (CryptoAssets, Heusenstamm), was für ein Nischen-Tool dieser Art typisch ist.
...
for t in TOOLS:
    print(f'  - {t["slug"]:14} {t["category"]:14} ov={len(t["overview"])}c  dsgvo={t["dsgvo"]}')
